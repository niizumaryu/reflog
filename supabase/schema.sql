...
-- ------------------------------------------------------------------
-- Hardening #3 (Round 8 audit): video_analyses.match_id itself had no
-- ownership check. RLS on video_analyses only verifies auth.uid() =
-- user_id on the row — it does not stop an authenticated user from
-- setting match_id to point at another user's matches row via a direct
-- REST call. Mirrors enforce_video_analysis_ownership() above, applied
-- to video_analyses.match_id.
-- ------------------------------------------------------------------
