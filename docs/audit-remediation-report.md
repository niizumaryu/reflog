...
# Round 8(2026-07-22実施、Round 5で一覧化のみだったP2/P3の消化)

Round 7までの内容を「最大60点の途中結果」として引き継いだ。開始時点で lint・型チェック・test 160件・build すべて成功のクリーンな状態を確認済み。今回は新規に問題を洗い出すのではなく、Round 5監査で「一覧化のみ・修正なし」として残されていたP2/P3項目のうち、視覚的な確認なしで安全に直せるもの(バックエンドロジック・DB制約・純粋関数)を優先して消化した。ブラウザ操作ツールがない制約は前回までと同じため、UI変更は控えめ(通知許可カードの追加のみ)にとどめている。

## P2: video_analyses.match_id に所有権検証トリガーがない(Round 5で発見・未対応のまま残存)

- **問題**: `video_analyses.match_id` は `matches.id` を参照する外部キーだが、RLSは行自体の所有者(`auth.uid() = user_id`)しか検証しておらず、`match_id` が実際に同じユーザーの `matches` 行を指しているかは検証していなかった。アプリのUIは常に `matchId: null` で作成するため悪用経路は現状存在しないが、認証済みユーザーが自分のJWTでSupabase REST APIを直接呼び出せば、他人の `matches.id` を `match_id` に設定できる状態だった。
- **対応**: `enforce_video_analysis_ownership`(子テーブル向け)と同じ設計の `enforce_video_analysis_match_ownership` トリガー関数を追加し、`video_analyses` の insert/update時に `match_id` が `null`、または `new.user_id` と同じ所有者の `matches` 行を指していることを強制するようにした。`supabase/migrations/20260722_enforce_video_analysis_match_ownership.sql`(新規、**本番未適用**)と `supabase/schema.sql` の両方に反映済み。

## P2: cronの`no_record_reminder`が同日複数試合で誤ったリマインドを送りうる(Round 5で発見・未対応のまま残存)

- **問題**: `src/app/api/cron/notifications/route.ts` の `no_record_reminder` は `.eq("date", ...).maybeSingle()` で「その日の記録があるか」を確認していた。`maybeSingle()` は2件以上ヒットすると例外を投げるが、呼び出し側は `error` を無視していたため、同じ日に複数試合を記録したユーザー(ダブルヘッダー等)では例外が握りつぶされて `existingMatch` が `null` のままになり、実際には記録済みなのに「記録を忘れていませんか」という誤ったリマインドが送られる状態だった。
- **対応**: `.maybeSingle()` を `.limit(1)` に変更し、配列の長さで存在判定するよう修正。あわせて、この判定に使っていた日時計算ロジック(JST基準の閾値計算)を `src/lib/notifications/noRecordReminder.ts` に純粋関数として切り出し、`src/lib/notifications/noRecordReminder.test.ts`(6ケース)で単体テストを追加した。ルート内に重複していた `jstWallClockToUtcMs` も `src/lib/date.ts` に統合し、`src/lib/date.test.ts` に3ケース追加(P3「日付フォーマット・重複実装」の一部もあわせて解消)。

## P2: Service Workerが非2xxレスポンスもキャッシュしていた(Round 5で発見・未対応のまま残存)

- **問題**: `public/sw.js` の fetch ハンドラーは、ネットワーク応答のステータスに関わらず(404・500含め)全てのレスポンスをキャッシュに書き込んでいた。デプロイ直後の一時的な500エラーや存在しないパスへの404を一度でも踏むと、以降オフライン時にそのエラーページがキャッシュから再生され続ける可能性があった。
- **対応**: `response.ok && response.type === "basic"` の場合のみキャッシュするよう修正。挙動が変わるため `CACHE_VERSION` を `reflog-v2` に更新し、デプロイ時に古いキャッシュが確実に破棄されるようにした。

## P2: 通知許可の初回リクエストに事前説明がなかった(Round 5で発見・未対応のまま残存)

- **問題**: `NotificationPermissionPrompt.tsx` は初回ログインを検知した瞬間に、何の説明もなくブラウザのネイティブ通知許可ダイアログ(`Notification.requestPermission()`)を呼び出していた。ブラウザのネイティブ許可ダイアログは一度拒否されるとアプリ側から再度求める手段がなく、文脈のない冷たい状態での要求は反射的な「ブロック」を招きやすいという、通知UXのベストプラクティス上の既知の問題だった。
- **対応**: ネイティブダイアログを呼ぶ前に、REFLOG自身の説明カード(画面下部、オレンジ基調・44pxタップ領域・`role="region"`)を表示するよう変更した。「許可する」を押した場合のみネイティブダイアログを呼び、「あとで」を押した場合はネイティブダイアログを呼ばず、14日間のクールダウン後に再度カードを表示できるようにした(`src/lib/notifications/settings.ts` の `isNotificationSoftAskDismissed`/`markNotificationSoftAskDismissed`、単体テスト3ケース)。ブラウザでの実機確認は本セッションでは実施できていない(引き続きブラウザ操作ツールがない制約のため)。

## P3: updateMatchの競合時に生のPostgrestエラーが表示されていた(Round 5で発見・未対応のまま残存)

- **問題**: `src/lib/matches.ts` の `updateMatch` は `.update(...).select().single()` を使っており、他タブで削除済みの試合記録を編集・保存すると `.single()` が "JSON object requested, multiple (or no) rows returned" という生の英語エラーをそのまま投げ、`src/app/matches/[id]/edit/page.tsx` がそれをそのままユーザーに表示していた。`updateSchedule`(Round 4で対応済み)と同種の欠陥が、対応漏れとして残っていた。
- **対応**: `updateSchedule`/`SCHEDULE_NOT_FOUND_MESSAGE` と同じパターンで `MATCH_NOT_FOUND_MESSAGE` を追加し、`.select()`(配列)+ 0件チェックに変更。編集ページはこのメッセージを検知した場合、保存失敗ではなく「見つかりませんでした」画面に遷移するよう修正した。回帰テスト3件を追加(`src/lib/matches.test.ts`)。

## P2: 認証ガードの重複14箇所がテストされていなかった(Round 3で発見・Round 5まで見送り継続)

- **問題**: `const { data: { user } } = await supabase.auth.getUser(); if (!user) throw new Error("ログインが必要です");` という同一パターンが、`matches.ts`・`schedules.ts`・`profile.ts`・`annualGoals.ts`・`notifications/center.ts`・`notifications/settings.ts`・`video-analysis/planUsage.ts`・`video-analysis/videoAnalyses.ts` の8ファイル14箇所に重複しており、どこにもテストがなかった。Round 3では「影響範囲が広く視覚回帰の確認が難しい」として見送られていたが、この処理自体はUIを持たないバックエンドロジックであり、機械的な置換であれば視覚回帰リスクはないと判断した。
- **対応**: `src/lib/auth/requireUser.ts`(新規)に共通化し、14箇所すべてを `const user = await requireUser(supabase);` に置き換えた。単体テスト2ケースを追加(`src/lib/auth/requireUser.test.ts`)。`if (!user) return null/0/false/DEFAULT` のように「未ログイン=エラーではなく既定値を返す」設計の箇所(`profile.ts`・`notifications/center.ts`・`notifications/settings.ts` に各1〜2箇所)は意図的に対象外とした(`requireUser` は常にthrowする設計のため、挙動が異なる)。

## 今回も対応を見送った項目(将来対応)

- トグルスイッチ(`NotificationToggle`等)のタップ領域28×48pxは、WCAG AA基準(24×24px)は満たしているが他要素の44px統一からは外れたまま。Round 4・6と同じ理由(ブラウザでの視覚確認ができない環境のため、一括変更のリスクを取らない判断)で今回も見送った。
- `src/app/report/page.tsx`(863行、プレゼンテーション・データ取得・ビジネスロジック混在)の分割は見送り。
- Stripe/PayPay/Apple課金/Google課金・ネイティブストア配信・チーム管理/大会管理/ランキング/コミュニティ等のロードマップ項目は、設計ドキュメント(`docs/billing-architecture.md`)の整理のみで実装は行っていない。本セッションの範囲外と判断した理由は最終報告を参照。

## テスト

新規テストファイル: `src/lib/notifications/noRecordReminder.test.ts`(6ケース)・`src/lib/notifications/settings.test.ts`(3ケース)・`src/lib/auth/requireUser.test.ts`(2ケース)。既存テストファイルへの追加: `src/lib/date.test.ts`(+3ケース)・`src/lib/matches.test.ts`(+3ケース)。既存160件(25ファイル)と合わせて計176件(28ファイル)、全て成功。`npm run lint`・`npx tsc --noEmit`・`npm run build`・Playwright `public-*`(52件、4ビューポート)すべて成功を確認済み。
